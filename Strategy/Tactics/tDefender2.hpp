#ifndef TDEFENDER2_HPP
#define TDEFENDER2_HPP
...
namespace Strategy
{
    class TDefender2: public Tactic
    {
		
	public:
		  int hasAchievedOffset;
		
			TDefender2(const BeliefState* state, int botID) :
			  Tactic(Tactic::Defender2, state, botID)
			{
			  iState = POSITIONING;
			} // TDWDefender	
			~TDefender2()
...
void execute(const Param& tParam)
{
	...
	case DEFENDING :           	// tAKES THE bot from behind
	{
	       if(state->ballPos.x  > oppthreshX )
		   {
			...
			// for ball in region below two cover goal 
			 if(ForwardX(state->ballPos.x) < ForwardX(-(HALF_FIELD_MAXX-GOAL_DEPTH)*0.8 + 2*BOT_RADIUS))
			 {       cout<<"Inside"<<endl;
			   	     sParam.GoToPointP.x = ForwardX(-(HALF_FIELD_MAXX-GOAL_DEPTH)*0.80 + 6*BOT_RADIUS);
                     sParam.GoToPointP.y = state->ballPos.y - SGN(state->ballPos.y)*4*BOT_RADIUS ;
					 skillSet->executeSkill(SkillSet::GoToPoint, sParam);
					 break ;
			 }
