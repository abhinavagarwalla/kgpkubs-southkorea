...
//    skillSet->executeSkill(sID, sParam);
  ...
  void Tactic::preciseKick(float angle, int speed) {
    ...
    if(fabs(angle - state->homeAngle[botID]) > PRECISE_KICK_THRESHOLD) {
  ...
  void Tactic::gotoStaticBall(const float finalSlope) {
    ...
    float angular_dist   = fabs(normalizeAngle( targetAngle - state->homeAngle[botID]));
    if(dist_from_ball < BOT_BALL_THRESH + 2*BOT_RADIUS) {
      ...
      if(fabs(target_angular_seperation) < PI/3)
  ...
  void Tactic::kickForGoalOrPassBall() {
    ...
    */
    if(ready_to_go) {
      ...
      if(fabs(normalizeAngle(targetAngle - state->homeAngle[botID])) > PASS_BALL_THRESHOLD) {
        turnToTargetWithBall(targetAngle);
      } else {
