...
//    te = Point2D<double>(0,0);
    ...
    Point2D<double> as;
    {
        ...
        std::cout << "d*ks = " << d*ks << ", radius = " << 1/ks;
        double t = asin(d*d*ks/d_2)+start.theta();
        as.x = cos(t)*d_2;
        as.y = sin(t)*d_2;
        // debugging,setting 0
        as = Point2D<double>(0,0);
        std::cout << "ks = " << ks << ", as = " << as.x << as.y;
    }
    ...
    Point2D<double> ae;
    {
        ...
        std::cout << "d*ke = " << d*ke << ", radius = " << 1/ke;
        double t = asin(d*d*ke/d_2)+end.theta();
        ae.x = d_2*cos(t);
        ae.y = d_2*sin(t);
        // debugging, setting 0
        ae = Point2D<double>(0,0);
        std::cout << "ke = " << ke << ", ae = " << ae.x << ae.y;
    }
    // P0 = start
    ...
    std::cout << "Points are (x): " << P5.x << P4.x << P3.x << P2.x << P1.x << P0.x;
    std::cout << "Points are (y): " << P5.y << P4.y << P3.y << P2.y << P1.y << P0.y;
    // coeff of polys:
    Point2D<double> a[6];
