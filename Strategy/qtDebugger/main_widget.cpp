...
#include "../HAL/fira_comm.h"
#include "../Utils/pathPlanners.h"
#include <vector>
#include "../../common/include/geometry.hpp"
#include "../../common/gpb/proto/cpp/sslDebug_Data.pb.h"
...
void MainWidget::paintEvent(QPaintEvent* event)
{
  ...
  // Drawing debug data
  std::list<Debug_Circle> circles = Strategy::SkillSet::getCircles();
  std::list<Debug_Line> lines = Strategy::SkillSet::getLines();
  ...
  {
    Debug_Circle circle = circles.front();
    circles.pop_front();
    QPoint centre(tr_x(circle.x(),fac), tr_y(circle.y(),fac));
    pen.setColor(circle.color());
    painter.setPen(pen);
    painter.save();
    painter.drawEllipse(centre, circle.radius()*fac, circle.radius()*fac);
    painter.restore();
  }
  while(lines.empty() == false)
  {
    Debug_Line line = lines.front();
    lines.pop_front();
