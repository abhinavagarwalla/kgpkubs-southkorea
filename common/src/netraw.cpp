...
bool UDP::addMulticast(const Address &multiaddr,const Address &inter_face)
{
  static const bool debug = false;
  struct ip_mreq imreq;
  imreq.imr_multiaddr.s_addr = multiaddr.getInAddr();
  imreq.imr_interface.s_addr = inter_face.getInAddr();

  if(debug){
    printf("0x%08X 0x%08X\n",
           (unsigned)inter_face.getInAddr(),
           (unsigned)INADDR_ANY);
  }
...
int main(int argc, char **argv)
{
  ...
  if(server == client) exit(EXIT_FAILURE);
